module.exports = {
    name: 'punish-list',
    dm: false,
    description: "Voir la liste des utilisateurs punish",
    type: "CHAT_INPUT",
    ...
    go: async (client, db, config, interaction, args) => {
        ...
        const owner = db
            .all()
            .filter((data) => data.ID.startsWith(`Punish_${interaction.guild.id}-`))
            .sort((a, b) => b.data - a.data);
        for (let i in owner) {
            if (owner[i].data === null) owner[i].data = 0;
            let userData = await client.users.fetch(owner[i].ID.split(`-`)[1])

            const t = db.get(`Punish_${interaction.guild.id}-${userData.id}`)
        ...
        interaction.reply({
            embeds: [
                {
                    title: "Liste des utilisateurs punish",
                    description: content,
                    color: 0x2E3136,
                }
            ],
